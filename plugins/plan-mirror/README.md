...
Keep the shell working directory at the project being reviewed. `report.md` and `result.json` are written under `.plan-mirror/runs/`; `candidate.md` is added only when a valid candidate is available. An invalid Fixer TASK format receives one format-correction retry. If that retry is also invalid, the run ends as `INCOMPLETE` with a report and without a misleading candidate.
...
Запускайте команды из корня проверяемого проекта. В `.plan-mirror/runs/` сохраняются `report.md` и `result.json`; `candidate.md` добавляется только при наличии корректного варианта. При неверном формате TASK Fixer получает одну попытку исправить формат. Если повторный ответ тоже неверен, запуск завершается как `INCOMPLETE` с отчётом и без вводящего в заблуждение кандидата.
